...
public class Menu {
	...
	public int verLista(ArrayList<Produto> lista_Produtos){
        int escolha;
		for (int i =0; i < lista_Produtos.size();i++){
			System.out.println((i + 1) + " " + lista_Produtos.get(i).getNome() + 
			", descrição: " + lista_Produtos.get(i).getDescricao() + ", R$ " +
			lista_Produtos.get(i).getValor());
		}
